<nav class="bg-white border-b border-stone-200 sticky top-0 z-40 px-6 py-4 flex items-center justify-between">
    ...
    <!-- Welfare Switcher -->
    <div x-data="{ open: false }" class="relative hidden md:block mx-auto">
        @if($activeWelfare)
            <button @click="open = !open" class="flex items-center gap-2 bg-stone-100 px-4 py-2 rounded-lg font-bold text-sm text-teal-900 border border-stone-200 hover:bg-stone-200 transition">
                {{ $activeWelfare->name }} 
                <i class='bx bx-chevron-down'></i>
            </button>
            
            <div x-show="open" @click.away="open = false" class="absolute mt-2 w-80 bg-white border rounded-xl shadow-xl p-2 z-50">
        ...
        @else
            <a href="{{ route('frontend.explore') }}" class="text-sm font-bold text-amber-600 hover:text-amber-700 transition">
    ...
    <div x-data="{ open: false }" class="relative">
        <button @click="open = !open" class="flex items-center gap-3 bg-stone-50 py-1.5 pl-2 pr-4 rounded-full border border-stone-200 hover:border-teal-900 transition">
            ...
            <span class="text-sm font-bold text-teal-900">{{ explode(' ', trim(auth()->user()->name))[0] }}</span>
