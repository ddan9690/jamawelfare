    ...
    <div x-data="{
        ...
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
            ...
            <div class="flex gap-4 w-full md:w-auto">
                <select x-model="selectedCounty"
                    class="flex-1 md:flex-none px-4 py-2 rounded-xl border border-stone-200 text-sm font-bold text-stone-700 outline-none focus:ring-2 focus:ring-teal-500">
                    <option value="all">All Counties</option>
                    @foreach ($counties as $county)
                        <option value="{{ $county->name }}">{{ $county->name }}</option>
                    @endforeach
                </select>
                <a href="{{ route('welfares.create') }}"
                    class="bg-teal-900 text-white px-6 py-2 rounded-xl font-bold text-sm hover:bg-teal-800 transition">Add
                    New</a>
            </div>
        </div>
            ...
            <table id="datatable" class="w-full text-sm">
                ...
                <tbody>
                    @foreach ($welfares as $w)
                        <tr x-show="selectedCounty === 'all' || selectedCounty === '{{ $w->county->name ?? 'N/A' }}'"
                            class="border-b last:border-0 border-stone-50">
                            <td class="py-3">
                                @if ($w->logo)
                                    <button
                                        @click="showLogoModal = true; selectedLogo = '{{ asset('storage/' . $w->logo) }}'">
                                        <img src="{{ asset('storage/' . $w->logo) }}"
                                            class="w-10 h-10 rounded-full object-cover shadow-sm border border-stone-100 hover:opacity-80 transition">
                                    </button>
                                @else
                                    <img src="{{ asset('images/welfare-placeholder.svg') }}"
                            ...
                            <td class="py-3 text-center">
                                <label class="relative inline-flex items-center cursor-pointer">
                                    <input type="checkbox" class="sr-only peer"
                                        ...
                                        onchange="confirmToggleStatus('{{ route('welfares.toggle-status', [$w->id, $w->slug]) }}', this)">
                                    <div
                            ...
                            <td class="py-3 text-right space-x-2">
                                <a href="{{ route('welfares.edit', [$w->id, $w->slug]) }}"
                                    class="text-amber-600 font-bold hover:underline">Edit</a>
                                <button onclick="confirmDelete('{{ route('welfares.destroy', [$w->id, $w->slug]) }}')"
        ...
        <!-- Modals -->
        <div x-show="showDescModal" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50"
            ...
            <div class="bg-white p-6 rounded-2xl max-w-sm w-full shadow-2xl">
                ...
                <button @click="showDescModal = false"
                    class="w-full py-2 bg-stone-100 rounded-lg font-bold text-sm text-stone-700 hover:bg-stone-200 transition">Close</button>
            </div>
        </div>

        <div x-show="showLogoModal" x-cloak class="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50"
    ...
    <script>
        $(document).ready(function() {
            $('#datatable').DataTable({
                ...
                "language": {
                    "search": "_INPUT_",
                    "searchPlaceholder": "Search welfares..."
                },
                "columnDefs": [{
                    "orderable": false,
                    "targets": [0, 3, 6]
        ...
        function confirmToggleStatus(url, checkbox) {
            let isChecked = checkbox.checked;
            let targetStatus = isChecked ? 'active' : 'suspended';
            checkbox.checked = !isChecked; // Revert visually until confirmed

            Swal.fire({
                title: 'Change Welfare Status?',
                text: `Are you sure you want to change the status to "${targetStatus}"?`,
                icon: 'warning',
                    ...
                    fetch(url, {
                        ...
                        .then(data => {
                            if (data.success) {
                                ...
                                Swal.fire({
                                    title: 'Success!',
                                    text: `Status successfully changed to "${data.status}"!`,
                                    icon: 'success',
                                    timer: 2000,
